...
ModelCnnExp4 = keras.models.Sequential([
    # this configuration is too expensive (too many parameters)
...
ModelCNNExp1 = keras.models.Sequential([
    # probably the most efficient one atm
...
## end --
print(f"Training {model_sel} model  with new configuration")
...
if model_sel == 'ModelCNNExp1' or model_sel == 'ModelCNNExp2':
    print(f"Training {model_sel} model  from scratch")
    model_new = keras.models.Sequential([
        ...
        # only use one dense 1024 unit layer atm
        # keras.layers.Dense(1024, activation='relu'),
        # keras.layers.Dropout(0.5),
    ...
    if frozen_weights:
        print(f"Perform training with forzen weight with {model_sel}")
    else:
        print(f"Perform training with no pretrained-weights {model_sel}")
...
checkpoint_path = f"{current_timestamp}_aug__no_imagenet_frozen_model_{model_sel}_AdamW_bs_{batch_size}_frozen_{str(frozen_weights)}_cosine_decay_scheduler.h5"
